...
function get_field_objects( $post_id = false, $options = array() ) {
    ...
    if ( is_numeric( $post_id ) ) {
        ...
        "SELECT meta_value FROM $wpdb->postmeta WHERE post_id = %d and meta_key LIKE %s AND meta_value LIKE %s", $post_id, '_%', 'field_%'
        ) );
    } elseif ( strpos( $post_id, 'user_' ) !== false ) {
        $user_id = str_replace( 'user_', '', $post_id );

        $keys = $wpdb->get_col( $wpdb->prepare(
        "SELECT meta_value FROM $wpdb->usermeta WHERE user_id = %d and meta_key LIKE %s AND meta_value LIKE %s", $user_id, '_%', 'field_%'
        ) );
    } else {
        $keys = $wpdb->get_col( $wpdb->prepare(
        "SELECT option_value FROM $wpdb->options WHERE option_name LIKE %s", '_' . $post_id . '_%'
        ) );
    }
...
function acf_form( $options = array() ) {
    ...
    <?php
    $output_post_id = $options['post_id'];
    if ( !is_numeric( $output_post_id ) ) {
        $output_post_id = "'" . $output_post_id . "'";
    }
    ?>
                acf.o.post_id = <?php echo esc_attr( $output_post_id ); ?>;
            </script>
